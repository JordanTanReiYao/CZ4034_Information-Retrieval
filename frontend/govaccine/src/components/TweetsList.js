...
export default function TweetsList(props) {
      ...
      .then((response) => {
        ...
        setSuggestions(
          response.data.suggestions.length > 0
            ? response.data.suggestions[1].suggestion[0]
            : []
        );
        const newSentiments = sentimentResults.map((sentiments) => {
          return {
            ...sentiments,
            value: response.data.docs.filter(
              ({ sentiment }) => sentiment === sentiments.label.toLowerCase()
            ).length,
          };
        });
  ...
  const handlePageClick = (e) => {
    ...
    setOffset(offset);
    setDisplayData(
      tweets.slice(e.selected * perPage, e.selected * perPage + perPage)
    );
  };
  ...
  return (
    ...
    >
      {loading ? (
        <Progress message="Retrieving Tweets" />
      ) : (
            ...
            displayData.slice(minValue, maxValue).map((tweet) => (
              <Card className="card">
                ...
                >
                  <div
                    className="row"
                    style={{ backgroundColor: "#92A8D1", padding: 5 }}
                  >
                    <div className="col-2">
                      <div style={{ width: "100%" }}>
                        <Tooltip
                          title={
                            <div>
                              <span
                                style={{ color: "lightblue", fontSize: 16 }}
                              >
                                Acct Desc: {unicodeToChar(tweet.acctdesc)}
                              </span>
                              <br></br>
                              <span
                                charset="UTF-8"
                  ...
                  >
                    <Typography
                      sx={{ fontSize: 17, mt: 2, mb: 3, width: "100%" }}
                      color="text.primary"
                      gutterBottom
                    >
                      {tweet.text}
                    ...
                    >
                      ...
                      >
                        <img
                          src={clock}
                          style={{ height: "29px", marginRight: "2.9px" }}
                        />
                        <Typography
                          sx={{ mb: 0 }}
                          align="right"
